...
read ebbr
echo -e "\e[32m是否开启sni-filter(y/n)?\e[0m"
echo -e "\e[33m[测试功能(将阻止偷跑指向cdn(CF)的流量,速度降低。REALITY不是指向CF的建议(no)]\e[0m"
read osni
old_flow=""
sni_filter=0
shitswim=0
...
#系统是 64 位架构
wget -P /usr/xray https://github.com/XTLS/Xray-core/releases/download/v24.11.30/Xray-linux-64.zip
...
h2ph=`openssl rand -hex 6`
h2nm=`openssl rand -hex 4`
h2path="old$h2ph"
h2name="old$h2nm.con"
...
{"log": {"loglevel": "warning"},"inbounds": [{
            ...
            "streamSettings": {
				...
				"httpSettings": {"path": "/$h2path","host": ["$h2name"]}}
		}
    ],
    "outbounds": [
        {
...
cp `which timeout` /usr/xray/snidaemon
echo "setsid /usr/xray/snifilter -listen $ipaddr $portx -sni $domain_s &" >> /usr/xray/xrayinit
echo "setsid /usr/xray/xray -c /usr/xray/sni_filter_config.json &" >> /usr/xray/xrayinit
echo "on" > /usr/xray/statusfilter
else
echo "[Unit]" > /etc/systemd/system/xray_service.service
echo "Description=xray Service" >> /etc/systemd/system/xray_service.service
echo "After=network.target" >> /etc/systemd/system/xray_service.service
...
echo 
echo "#!/bin/bash" > /usr/xray/snifilter
echo "if [ -z \"\$2\" ]; then" >> /usr/xray/snifilter
echo "exit -1" >> /usr/xray/snifilter
echo "fi" >> /usr/xray/snifilter
echo "while true" >> /usr/xray/snifilter
echo "do" >> /usr/xray/snifilter
echo "/usr/xray/snidaemon 1h /usr/xray/sni-filter \$* > /dev/null 2>&1" >> /usr/xray/snifilter
echo "done" >> /usr/xray/snifilter
echo "#!/bin/bash" > /usr/xray/opensni
echo "echo \"#!/bin/bash\" > /usr/xray/xrayinit" >> /usr/xray/opensni
echo "echo \"setsid /usr/xray/snifilter -listen $ipaddr $portx -sni $domain_s &\" >> /usr/xray/xrayinit" >> /usr/xray/opensni
...
echo "echo \"on\" > /usr/xray/statusfilter" >> /usr/xray/opensni
echo "echo 已打开,正在重启xray" >> /usr/xray/opensni
echo "systemctl restart xray_service" >> /usr/xray/opensni
echo "olddy=\"$realip:$portx?encryption=none&security=reality&sni=$domain_s&fp=chrome&pbk=$public_old&sid=$shortIds&type=http&host=$h2name&path=%2F$h2path#xray_REALITY\"" >> /usr/xray/opensni
echo "olduuid=\`cat /usr/xray/oldf_uuid.json\`" >> /usr/xray/opensni
echo "echo 新订阅为: vless://\$olduuid@\$olddy" >> /usr/xray/opensni
echo
mkdir /run/old
chown xrayuser /run/old
chmod 655 /usr/xray/sni*
echo "killall sni-filter > /dev/null 2>&1 " > /usr/xray/xraynobug
echo "echo -e \"\033[32mbug已解决\033[0m\"" >> /usr/xray/xraynobug
else
echo "#!/bin/bash" > /usr/xray/chaguuid
echo "newuuid=\`/usr/xray/xray uuid\`" >> /usr/xray/chaguuid
echo "olduuid=\`cat /usr/xray/oldf_uuid.json\`" >> /usr/xray/chaguuid
...
echo "systemctl restart xray_service" >> /usr/xray/chaguuid
echo "olddy=\"$realip:$portx?encryption=none&security=reality&sni=$domain_s&fp=chrome&pbk=$public_old&sid=$shortIds&type=tcp&headerType=none&host=$domain_s&flow=$old_flow#xray_REALITY\"" >> /usr/xray/chaguuid
echo "olddy2=\"$realip:$portx?encryption=none&security=reality&sni=$domain_s&fp=chrome&pbk=$public_old&sid=$shortIds&type=http&host=$h2name&path=%2F$h2path#xray_REALITY\"" >> /usr/xray/chaguuid
echo "echo uuid已更新,新uuid为: \$newuuid" >> /usr/xray/chaguuid
echo "if [ -e \"/usr/xray/statusfilter\" ]; then" >> /usr/xray/chaguuid
echo "echo 新订阅为: vless://\$newuuid@\$olddy2" >> /usr/xray/chaguuid
echo "else" >> /usr/xray/chaguuid
echo "echo 新订阅为: vless://\$newuuid@\$olddy" >> /usr/xray/chaguuid
echo "fi" >> /usr/xray/chaguuid
echo "#!/bin/bash" > /usr/xray/opensni
echo "echo -e \"\033[31msni-filter未安装\033[0m\"" >> /usr/xray/opensni
cp /usr/xray/opensni /usr/xray/closedsni
echo "echo -e \"\033[31mbug不存在\033[0m\"" > /usr/xray/xraynobug
fi
...
echo "killall snifilter ; killall sni-filter > /dev/null 2>&1" >> /usr/xray/delxray
fi
...
echo "if [ -e \"/usr/xray/statusfilter\" ]; then" >> /usr/xray/xraystop
echo "killall xray > /dev/null 2>&1" >> /usr/xray/xraystop
echo "killall snifilter ; killall sni-filter > /dev/null 2>&1" >> /usr/xray/xraystop
echo "systemctl stop xray_service" >> /usr/xray/xraystop
echo "else" >> /usr/xray/xraystop
echo "systemctl stop xray_service" >> /usr/xray/xraystop
echo "fi" >> /usr/xray/xraystop
echo "#!/bin/bash" > /usr/xray/xraystart
echo "systemctl start xray_service" >> /usr/xray/xraystart
echo "#!/bin/bash" > /usr/xray/xrayrestart
echo "if [ -e \"/usr/xray/statusfilter\" ]; then" >> /usr/xray/xrayrestart
echo "killall xray > /dev/null 2>&1" >> /usr/xray/xrayrestart
echo "killall snifilter ; killall sni-filter > /dev/null 2>&1" >> /usr/xray/xrayrestart
echo "systemctl restart xray_service" >> /usr/xray/xrayrestart
echo "else" >> /usr/xray/xrayrestart
echo "systemctl restart xray_service" >> /usr/xray/xrayrestart
echo "fi" >> /usr/xray/xrayrestart
...
ln -s /usr/xray/xraynobug /usr/bin/xray.debug
...
echo
if [ $sni_filter -eq 1 ]; then
echo -e "\e[32mvless://$id_s@$realip:$portx?encryption=none&security=reality&sni=$domain_s&fp=chrome&pbk=$public_old&sid=$shortIds&type=http&host=$h2name&path=%2F$h2path#xray_REALITY\e[0m"
else
